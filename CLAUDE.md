...
- `!! <intent>` — Claude suggests a command based on intent, then offers to run
  it in the sandbox (`run in sandbox? [y/N]`, default No). The suggestion is
  never executed automatically — only on an explicit yes.
